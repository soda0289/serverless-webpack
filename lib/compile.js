...
module.exports = {
  compile() {
    ...
    const disableMultiCompile = true;

    const webpackConfigs =
      !disableMultiCompile || _.isArray(this.webpackConfig) ? this.webpackConfig : [this.webpackConfig];

    return BbPromise.mapSeries(webpackConfigs, webpackConfig => {
      const compiler = webpack(webpackConfig);

      return BbPromise.fromCallback(cb => compiler.run(cb)).then(stats => {
        if (!this.multiCompile || disableMultiCompile) {
          stats = { stats: [stats] };
        }
